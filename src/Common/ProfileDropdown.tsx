...
const ProfileDropdown = () => {
    ...
    const logout = () =>{
        axios.post(`http://localhost:8800/companies/logoutCompany/${user._id}`,{})
        .then((res: any)=> {
          console.log(res);
          Cookies.remove('astk');
          navigate("/login")
            ...
            <Dropdown className="ms-sm-3 header-item topbar-user">
                <Dropdown.Toggle type="button" className="btn bg-transparent border-0 arrow-none" id="page-header-user-dropdown">
                    <span className="d-flex align-items-center">
                        <img className="rounded-circle header-profile-user" src={`http://localhost:8800/CompanyFiles/logoFiles/${user.logo_file}`} alt="Header Avatar" />
